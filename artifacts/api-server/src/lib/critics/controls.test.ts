...
test("audit Appendix A, Probe 1: the random-pitch composer is flagged by harmony and voiceLeading with major/blocking severity", () => {
  const rows: string[] = [];
  ...
  assert.ok(voiceLeadingFlagged >= 2, `voiceLeading flags major on ${voiceLeadingFlagged}/${cases.length} (steps <= 7 keep most lines smooth; the leaps come from the voicing matcher)`);
});
...
test("the ledger is derived from the harness and the evidence is written from the same run", { timeout: 20 * 60 * 1000 }, () => {
  ...
  if (WRITE) {
    ...
    const evidence = {
      ...
      method: "every critic dimension over every anchor x control; detected = score drop >= 1 and a located non-info observation new or upgraded on the damaged tracks; rate with exact Clopper-Pearson 95 % interval",
