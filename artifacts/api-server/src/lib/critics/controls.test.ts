  ...
  if (WRITE) {
    ...
    const evidence = {
      ...
      recalibratedAtMerge: "onto main with B-00 (shipped-notes score, composer split), B-01 (ArrangementArc: drums enter at the chorus, bass thinned in quiet sections, chorus 2 developed, keys in every section, ensemble/mix not a family) and B-03 (instrument profiles); see the PR-B05a tracker entry. " +
        ...
        "Regenerated again at the second reconciliation with B-18, one cause and no change to any control: `writeBassLine` writes its approach tone on the last beat of the chord it leaves (where `realiseBassLine` writes its own) instead of on whichever groove onset happened to be last before the change, and takes its pitch from B-18's `approachToneChoice` instead of a copy of the pre-B-18 rule - so seven of the nine anchors' bass lines moved and the anchors were re-measured against them. `dimensions/controlLedger.ts` came out byte-identical: no control changed tier.",
