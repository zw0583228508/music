  ...
  return {
    ...
    d3_approachTones: {
      ...
      after: {
        ...
        universalRule: {
          claim: "no style approaches a minor chord through its own major third - not the chord it sounds over, not the chord it leads to",
          why: "R-1b P1-6 names the note by the chord it sounds over (\"E natural under Cm\"): it sits a semitone from the minor third the keys hold for the whole beat it sounds, and resolving a beat later does not undo it. B-05c's harmony critic reads it the same way and grades it `major` whatever the style.",
          foundOn: "jazz-full, after B-18's own no-brief groove reading moved its bass: an E natural over Cm7 as a chromatic approach into the next chord's F. The harmony dimension scored 78.4 with two `approach_tone_wrong_mode` majors; with the rule it scores 89.2 and both drop to `minor` (out-of-chord-mode chromatic approaches, which R-1b calls the jazz idiom).",
        },
        goldenImpact: "ballad-piano-vocal, ethnic-vocal and cinematic-midi composer digests moved with identical note counts; acoustic-demo's shipped notes moved; jazz-full moved for the groove reading and again for the universal rule; the other chromatic-style cases are byte-identical (see reference-part-composer.golden.json recordedAt)",
      },
      plannerRead: {
        ...
        callPassesBothChords: "sourceChord: event.chord, targetChord: events[i + 1]?.chord ?? null - on the same line",
      },
    },
    ...
    honestLimits: [
      ...
      "The style vocabularies judge an approach against the *key's* mode; B-05c's harmony critic judges it against the *chord's* own mode. Only the strongest case - the major third of a minor chord - is enforced here in every style. A chromatic style's other out-of-chord-mode approaches remain (jazz-full keeps two, graded `minor`), which is the idiom R-1b explicitly blessed for jazz, and which leaves jazz-full's harmony score at 89.2 against B-05c's `>= 90` pin for a clean anchor - so `critics/dimensions/harmony`'s null control fails (1 of 5). Adding jazz-full to that test's `belowNinety` map would make it green in one line; this stream refused, because the score fell for a change made here and registering a regression in the gate is not fixing it. That is the lead's call, not this stream's.",
      "Five of B-05c's tests fail with this stream on top of it, in three suites, and the cause is D1 working: `critics/dimensions/ownerAnchor` 3 of 5, `critics/dimensions/harmony` 1 of 5 (the jazz-full pin above), `critics/rank` 1 of 12 - while `critics/controls` 5/5, `critics/judge` 10/10, `critics/adversarial/adversarial` 43/43, `critics/b05cEvidence` 1/1, `critics/dimensions/density` 7/7 and `critics/dimensions/groove` 6/6 stay green. With the global -1 marking gone, the owner's sections rise one step (verses pp -> p, choruses mp -> mf), and `sectionPhrasePlanner.assignRole` - which reads the section's level and not the family's - assigns the strings HARMONIC_BED instead of PAD, so the anchor's track id is `strings-harmonic_bed`. The findings themselves survive intact: `single_voice_bed` still fires on the strings in the same six sections (meanVoices 1.00-1.17), so nothing was hidden. Renaming the id in B-05c's assertions settles one of the three ownerAnchor failures and no more: control A needs the strings' composed off-grid findings in >= 3 sections and now sees 2, and the string-bed test needs >= 4 sections with the composed strings in view and now sees 2. `critics/rank` puts the empty two-bar intro back into the top twelve at #9 against a `>= 12` pin, though the two orderings R-1b P0-5 actually asked for still hold (the bed and the off-grid harmony both outrank it). The root remedy is the per-family read in `sectionPhrasePlanner.assignRole`, listed for B-07: with it the strings are a PAD again and the anchor is the one B-05c measured. It is a file this stream does not own and it would move the golden corpus a third time, so it is handed to the lead rather than taken here - and no count this stream caused to move was re-pinned.",
