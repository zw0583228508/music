 ...
 *        [--source docs/evidence/listening-benchmark-v2-report.json] [--out docs/evidence/positive-control-ledger.json]
 *        [--no-tier-h] [--tier-p] [--seed 7]
 *
 ...
 * the real PDMX passages of the Listening Benchmark V2 report, rebuilt from
 * the local PDMX MIDI when it is present (each is a human part in its human
 * context); Tier P with `--tier-p` — the operator's song, reported but never
 * pooled with the others. Every metric the repo has is run on the original and
 * on each corrupted copy; the ledger says which metric may gate which family.
 *
...
const sourcePath = resolve(repoRoot, flag("source", "docs/evidence/listening-benchmark-v2-report.json"));
...
ledger.source = {
  ...
  tierH: { ...tierHSummary, from: relative(repoRoot, sourcePath) },
