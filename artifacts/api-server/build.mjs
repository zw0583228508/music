...
import { formatHostErrorMessage } from "./scripts/host-error-diagnostics.mjs";
...
async function buildAll() {
  ...
  await esbuild({
    entryPoints: [
      ...
      // Brain B-07: the evaluation + export render worker thread (renderOffThread.ts).
      path.resolve(artifactDir, "src/render-worker.ts"),
...
buildAll().catch((err) => {
  console.error(formatHostErrorMessage(err, "API build failed"));
  process.exitCode = 1;
});
