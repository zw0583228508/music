...
**Tests.** `arrangementBenchmark.test.ts` 11 (an unselectable case is reported, counted and compared as a regression; incl. the tritone positive control of the harmony measure, refusal across pins/tiers/render/corpora, Tier P as its own run), `benchmarkMeasures.test.ts` 7, `positiveControlLedger.test.ts` 7, `realCorpusBenchmark.test.ts` 8 (measurement, refusal on metre changes, rarest-first selection, **contested and score-licence-only works excluded before the quota fill and reported with reasons; one entry per composition**, N-family stripping, the run with a silent provider scoring a real zero), `compositionRights.test.ts` 5 (list consistency — every composer dead by 1955; spelling folds; ABBA / Coolio / Croce / Fernández / Misc Computer Games / medleys / placeholders contested with reasons; traditional label not proof — Santa Baby refused, Greensleeves and a labelled chant admitted; arranger credits), `benchmarkCorpusPlan.test.ts` 6 (a public-domain claim on the score's licence alone refused, Sibelius d. 1957 refused, contested refused, **the generated Tier H holds no contested entry and every entry proves its composition**), `pdmxIngest.test.ts` 6 (a row with no verified composer refused as contested), `blindListening.test.ts` 6 (no controls → withheld; controls not heard → withheld `not_sensitive`; five raters → `insufficient_data`; eight raters detecting → allowed; control votes outside Elo/tallies; context bytes identical across controls; leak probes) — 56 tests, all registered in `run-focused-api-tests.mjs`; the existing `tournamentListening`, `listeningBenchmarkV2`, `listeningSensitivity` suites still pass (13). `pnpm run typecheck` green.
