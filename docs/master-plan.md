...
### PR-B08 — Brain B-08: the measure before the redesign

Stream B-08 of the Arrangement & Orchestration Brain (`docs/brain/02-diagnosis-and-dag.md`), specified by the evaluation audit `docs/brain/reviews/2026-09-10-audit-evaluation-and-loops.md` §7. Nothing in the arrangement path changed; what changed is what gets measured, how a baseline is frozen, which metric is allowed to gate, what the real corpus is, and what a Gate C room has to prove before its verdict counts. Base `3bf23aa`.

**What shipped.**
- **The measure (audit §7.1).** `arrangementBenchmark.ts` 2.0 with a new pure module `benchmarkMeasures.ts`. `harmonyScore` is now computed on the **selected candidate's performed notes** — duration-weighted chord-tone share against the Song Model chords minus the share of sounding time a semitone from another sounding part, both through the tournament judge's own `chordToneShare` / `contextClashShare`; the 1.0 number (the critic's `harmony` dimension, a property of the Song Model — 58.33 for every composer ever measured) survives as `sourceHarmonyScore`, informational. `shippedCriticScore` runs `critiqueArrangement` on the performed tracks the candidate returns, beside the orchestrator's `criticScore`. `sectionConsistency` (100 on every run) and `candidateDiversity` (50.4 on every run) are replaced by the coherence metric's `trajectorySmoothness` / `seamArtefacts` / `motifRecurrence` on the selected candidate with the plan's section targets, and by `candidateDistance` (the production diversity gate's own mean pairwise distance, ×100); both old numbers stay one release as `legacy*`. Every run pins `gitSha`, `tier`, `render`, `corpusIds` and `metricVersions` (musicCritic 1.0, audioCritic 1.0, partJudge 1.1, COHERENCE_METRIC_v1, benchmarkMeasures 1.0); `compareBenchmarkRuns` returns `incomparable` across any of them — a comparison across a metric change measures the metric, not the music.
- **Baseline procedure (audit §7.3).** `benchmark-cli.ts`: `--out <file>`, `--compare <baseline.json>` (verdict table, exit 1 on `do_not_promote` unless `--no-fail`, exit 2 when incomparable — it prints which pin differs), `--tier S|P`, `--candidates N`; repo-relative paths resolve against the workspace root. Baseline committed: `docs/evidence/benchmark-baseline/3bf23aa.json` (Tier S, `--render`, 9 cases × 5 candidates). **Re-snapshot once PR-98 (playability repair in the perform stage) merges** — the shipped notes change there and `shippedCriticScore` is meant to show it: `pnpm --filter @workspace/api-server run benchmark -- --render --out docs/evidence/benchmark-baseline/<new-sha>.json`, then compare every later change with `pnpm --filter @workspace/api-server run benchmark -- --render --compare docs/evidence/benchmark-baseline/<new-sha>.json`.
- **Positive-control ledger (audit §7.4).** New `positiveControlLedger.ts` + `pnpm --filter @workspace/api-server run control-ledger` → `docs/evidence/positive-control-ledger.json`. Anchors: the nine synthetic cases' selected candidates (Tier S, 40 target parts) and the 21 real PDMX passages of the Listening Benchmark V2 report rebuilt from the local MIDI (Tier H, human parts in their human context). Every family of `symbolicCorruptions.ts` (19 × 3 severities) and every rung of `listeningDegradations.ts` (6) is applied to each target; every available metric — the 11 `musicCritic` dimensions and overall, the new benchmark harmony measure, `partJudge` and its components, `coherenceMetric` and its components, `evaluateCandidateMusicalFit` and its 17 dimensions — is run on the original and the corrupted copy. Rule (the sensitivity gate's own constants): **gate** ≥ 90 % detection at the strongest rung on ≥ 8 trials; **inform** above chance (one-sided exact p < 0.05); otherwise **demoted**. 79,110 applicable trials in 19 s.
- **Real corpus (audit §7.2).** `REAL_BENCHMARK_CORPUS` holds 40 Tier H entries generated by `scripts/select-real-corpus.mjs` into `realCorpusTierH.ts` (evidence `docs/evidence/real-corpus-tier-h-selection.json`): the whole admitted PDMX table streamed (254,077 rows, 222,856 admitted, 25,414 multitrack), every multitrack MIDI on disk **measured** (`realCorpusSelection.ts`: one metre per work or refused, first tempo, bars, parts, families, onsets per bar, swing from off-beat onset phases; genre/tags for idiom; CSV pitch-class entropy for harmony), 7,633 candidates, **then the composition-rights step (6/6, below — only 901 candidates prove their composition)**, then the repo's tournament-anchor works first and every required value filled rarest-first to `MIN_PER_VALUE` from the proven works, one entry per composition: 4/4 ×19, 3/4 ×8, 3/8 ×3, 6/8 ×3, 12/8 ×2, 3/2 ×2, 2/2 ×2, 2/4 ×1; swung ×3 (notated triplet subdivision, not a jazz feel), compound ×5; dense ×25, moderate ×12, sparse ×3; large ×25, small ×15; slow ×21, medium ×15, fast ×4; complex ×11, moderate ×21, simple ×8; **non_western 0 of 3** (a gap, not filled). `CorpusEntry.symbolicSource` (work id, path, sha256, measurements, how the repo admitted it). The **Tier H task** is stated once (`TIER_H_TASK`, "strip N families → arrange → compare against the human original at the part level", tournament task shape) and implemented: `runRealCorpusBenchmark` (`pnpm … run real-corpus` → `docs/evidence/real-corpus-tier-h-run.json`) runs the reference composer on every entry with material, 16-bar windows, one task per GM program, judged by `judgePart` against the human anchor; `stripContextFamilies` implements N > 1. `PROFESSIONAL_HUMAN_GOLD` stays blocked (`levelBlocker` unchanged). **Tier P**: the owner's song "רחם נא" as a slimmed fixture (`__fixtures__/rachemNaSongModelV3.ts`, 91 KB: tempo/meter/key maps, 92 chords, 9 sections, 141 bars, beats, energy, vocal evidence, the musical map's harmony/energy/vocals summaries; project ids, reconciliation, provenance and storage paths dropped), rights `owned_by_operator`, `--tier P` runs it as its own run; tiers never compare.
- **Human A/B controls (audit §7.5).** `blindListening.ts` 1.1: every candidate session carries three `HUMAN_DEGRADED` control pairs (pitch_shift 60 %, 30 %, 60 %) whose pair ids, case ids and questions are indistinguishable from the song pair's (the comparison lives in owner-only `meta`; the leak-probe list passes); `buildCandidateControls` writes the challenger's tracks as one MIDI with the most active pitched track last and degrades only that track through `degradeSideMidi` (context bytes identical across every control); `sessionResults` **withholds Gate C** unless `sensitivityReport(session, independent votes, primary question).gate.verdict === "may_judge_training"` — a session with no control pairs is withheld with that reason — and control votes never enter the Elo, the tallies or the release share. `gateC.sensitivity` added to the result and to `ListeningGateC` in `openapi.yaml` (orval regenerated). `POST /projects/{id}/listening-sessions` builds the pairs, renders original + degraded copies with `LISTENING_SYNTH_V2`, stores them under the session (`audioByToken`); when the challenger has no track models or the project no Song Model tempo the session is created without controls and Gate C says so.
- **Rights correction (6/6, the lead's ruling of 2026-09-10).** The 40 Tier H entries of 3/5 were admitted on the PDMX row's CC0 / public-domain statement alone. That statement is the **uploader's**, and clears at most the engraving or arrangement — never the composition ("Mamma Mia!" under CC0 is still ABBA's song); the standing rule (charter §2.2, `benchmarkCorpusPlan.ts`: a dataset's licence is not proof of rights in the works inside it) was being broken. The corpus now **fails closed on the composition**: `CorpusRightsBasis` is a union — `public_domain` requires `composer` + `composerDied` (year ≤ `PUBLIC_DOMAIN_DEATH_YEAR_CUTOFF` = 2026 − 70 − 1 = **1955**) or `traditional: true` + `source`; a new `contested` kind (`commercialUse: false`, `reason`) records a work the corpus looked at and could not clear. `corpusRefusalReason` refuses a public-domain claim with no composition evidence ("claims public domain on the score's licence alone"), a composer dead after the cutoff, a traditional claim with no source, and every contested basis. The mechanism is **admit only what is proven**, never a denylist: new `compositionRights.ts` holds a curated list of **{VC} public-domain composers** with death years and the spellings met in PDMX (Renaissance polyphonists through Ravel / Holst / Vierne / Rachmaninoff / Prokofiev; nothing after 1955 — Sibelius d. 1957 is absent) and **{VT} verified traditional / public-domain tunes** (Silent Night → Gruber d. 1863, Adeste Fideles → Wade, Greensleeves → English 1580, Jingle Bells → Pierpont …); `compositionRightsFor` reads the row's `composer_name` / `artist_name` (new `PdmxCsvRow.artist`) and title — dates, keys, punctuation and diacritics folded, `arr.` credits split off — and returns a basis or a reason (a multi-word name the list does not know blocks; a single-word handle names nobody but does not block a verified title; a "Traditional" / "Misc Christmas" label is not proof unless the title is on the tune list — "Santa Baby" and "Mary Did You Know" arrive under those labels; "Misc Computer Games" is a category, not a composer). `pdmxRightsBasis` now returns `public_domain` with the composition basis or `contested`; `pdmxToCorpusEntries` refuses contested rows with the reason; `selectTierH` 2.0 runs `admitEntries` **before any quota is filled** and returns `excluded[]` — a contested work leaves its slot open or short, never filled by the next unproven work — and collapses further arrangements of one composition (the first proven-only run held two "Stille Nacht", two "Canon and Gigue"). Both layers are stated per entry: the composition through the basis, the arrangement / engraving through the uploader's dedication in `no_license_conflict` — a living arranger's CC0 arrangement of a public-domain work is admitted on those two facts; an arrangement of an unproven composition is contested whatever its score licence. **Result:** of 7,633 measured candidates **901 prove their composition** (872 by composer, 29 by tune) and **6,732 are contested** (4,661 name a person not on the list, 991 carry only a handle, 742 name nobody, 199 wear a traditional label on an unlisted title, 139 a category label); **36 of the previous 40 entries and 49 of the 54 tournament seeds are contested** and listed in the report's `excluded[]` with title and reason — "Mamma Mia!" (names Andersson / Anderson / Ulvaeus), "Gangsta's Paradise" (Rickey C Battle Jr, the arranger, in the composer field), "Señorita" (Camila Cabello), "i will survive" (Gloria Gaynor), "country roads" (Hermes House Band), "time in a bottle" (Jim Croce), "earth angel" (Marvin Berry / The Starlighters), "sandstorm" (Darude), "Nipsey Hussle x Victory Lap" (Tyquon Garrett), "guantanamera" (Joseíto Fernández, d. 1979), "let me down slowly" (Alec Benjamin / Alessia Cara), "Pure Cocaine" (Lil Baby), "I Can't Breathe" (Jordan Jennings), "earthbound - onett" (Misc Computer Games); the four kept are Silent Night (Gruber), Symphony No. 9 (Beethoven), Vierne's Benedictus and Sullivan's Christ is Risen. Every contested candidate of the scan is in the sidecar `docs/evidence/real-corpus-tier-h-contested.json` (6,732 rows, composer / artist / reason), so the gap is visible rather than filled. **The new Tier H: 40 works, 31 composers** — Renaissance 7 (Dufay d. 1474, Lasso, Pallavicino, G. Gabrieli, Sweelinck, Leoni, d'India), Baroque 11 (Huygens, Purcell, Pachelbel, Buxtehude, Vivaldi, Bach ×5, Handel), Classical 7 (Mozart ×4 — K.467, K.477, K.618, K.626 — Haydn, Beethoven, Schubert), Romantic and after 13 (Gruber, Mussorgsky, Wagner, Liszt, Gounod, Verdi, Sullivan, Parry, Saint-Saëns ×2, Fauré, Vierne ×2), traditional 2 (Greensleeves, Bring a Torch); latest death year 1937; 35 from the CSV scan, 5 tournament seeds. The positive-control ledger's Tier H anchors now come from this corpus (`run-control-ledger.mjs --source corpus`, default: the Tier H task windows of every entry, 2 per work) instead of the Listening V2 report's 12 score-licence works. The idiom regex lost its bare `world` (it had labelled three Christmas carols non_western through the tag "joytotheworld") and is bounded by non-letters; under-counting is the honest direction.

**Evidence.** `docs/evidence/brain-b08-benchmark-measure.json` (before/after on the same sha, ledger lists, corpus coverage, Tier H run, limits); `docs/evidence/benchmark-baseline/3bf23aa.json`; `docs/evidence/positive-control-ledger.json`; `docs/evidence/real-corpus-tier-h-selection.json` (with `rights`, `excluded[]`); `docs/evidence/real-corpus-tier-h-contested.json` (every contested candidate); `docs/evidence/real-corpus-tier-h-run.json`.
- *Baseline, same sha, old vs new fields (Tier S, render on).* Old: criticScore 74.56 · harmonyScore 58.33 · sectionConsistency 100 · candidateDiversity 50.4 · audioScore 90.33 · playabilityErrors 0 · **renderFailures 5 on every case** (the render baseline of 2026-09-08 never reported it). New: criticScore 74.56 · **shippedCriticScore 74.67** · harmonyScore 98.43 (chordToneShare 98.9 %, clashShare 0.48 %) · sourceHarmonyScore 58.33 · trajectorySmoothness 48.03 · seamArtefacts 50.7 · motifRecurrence 33.16 · candidateDistance 6.59 · legacy 100 / 50.4. The positive control the 1.0 measure never had: the reference composer with every pitched part moved a tritone (same rhythm, same register, every note off the chord) scores harmonyScore **0 on eight cases and 9.7 on jazz-full (aggregate 98.35 → 1.07)** while the orchestrator's criticScore moves **74.56 → 73.56** and rises on two cases — the audit's F1 as a number. 98 is the reference composer stacking chord tones (F6), not a good harmony. Symbolic runs select different candidates than render runs (audio weighs 0.4) and are refused for comparison. Tier P (owner's song, symbolic): critic 70 = shipped 70, harmonyScore 99.87, sourceHarmonyScore 88, motifRecurrence 0, candidateDistance 7.08, legacySectionConsistency 75.
- *Ledger.* May gate: `benchmarkHarmony.chordToneShare` on 10 families including both listening pitch rungs (50/50 on S and H alike), `benchmarkHarmony.harmonyScore` on 12, `clashShare` on 3; `partJudge.playabilityErrors` / `rangeShare` / `registerShare` and `candidateQuality.playability` on `role_inversion` only; `candidateQuality.repetition` on `bar_copy_repetition` and `parallel_doubling`; `partJudge.score` on 15 (biased by its anchor-relative terms — read its components); `musicCritic.overall` and `musicCritic.playability` on `density_doubling` and `role_inversion`. Inform: `coherence.score` on 10 families, `coherence.harmonicAgreement` on 4, `coherence.motifRecurrence` on 3, `partJudge.coverage` on `density_thinning`, `partJudge.barRepetitionShare` on `bar_copy_repetition`. **Demoted (detect no family): `musicCritic.harmony`, `groove`, `voiceLeading`, `leadCompatibility`, `orchestration`, `sectionDevelopment`, `motifCoherence`, `contrast`, `transitions`, `performancePotential`** (ten of eleven), `coherence.seamArtefacts`, `coherence.instrumentationContinuity`, `coherence.trajectorySmoothness`, `candidateQuality.registerCollisions`. Physical playability does **not** gate `leap_injection` (7/32) or `octave_displacement` (18/50): a leap or a displaced octave that stays inside the GM extended range is legal to the engine. Nothing automated hears `onset_jitter` at gate level (best: chordToneShare informs at 40/50, because a jittered note straddles a chord change). 13 `candidateQuality` dimensions are unavailable on brain output and are listed as insufficient data.
- *Ledger re-run on the proven corpus (6/6).* Anchors 9 Tier S + **80 Tier H** (2 task windows per proven work; was 21 windows of 12 score-licence works), 120 targets, **129,834 applicable trials in 63 s**. Tier S anchors are the same nine candidates and no Tier S-only verdict changed (the corruption RNG stream is shared across anchors in order, so 73 rows' Tier S detected counts move by a trial or two). 38 of 1,035 rows changed verdict, all on families with Tier H trials. Gate gained: `benchmarkHarmony.clashShare` on `pitch_shift_in_key`, `phrase_shift`, `section_swap`, `listening_random_pitch` (now 7 families); `partJudge.score` on `quantisation_coarsening`; `partJudge.coverage` on `density_thinning`; `partJudge.barRepetitionShare` on `bar_copy_repetition`. Gate lost (to inform): `chordToneShare` on `bar_copy_repetition` and `phrase_shift` (now 8 families); `partJudge.score` on `syncopation_removal` and `motif_destruction` (now 14). `coherence.trajectorySmoothness` leaves the demoted list (informs on `leap_injection`; **demoted 14 → 13**); `coherence.score` informs on 6 more families (16), `coherence.harmonicAgreement` on 8 more (12), `coherence.motifRecurrence` on 2 (lost 3). Onset jitter is still heard by nothing at gate level. The ten `musicCritic` dimensions stay demoted — they run on Tier S only and Tier S did not change.
- *Tier H run (3/5, superseded — that corpus was admitted on score licences; 36 of its 40 works are now contested).* 40 works, 153 tasks, 0 failures, judge 1.1. Reference composer mean 49.2 vs the human's own part 93.4 on the same windows (Δ −44.2); at or above the human on 2 % of tasks (3 of 153 — all drums, where the judge is suspect: no chord/interval terms apply and the human kit exceeds the GM ceilings); playability errors 0.72 per task, chord-tone share 0.96. By family: ensemble −79, reed −63, pipe −55, brass −48, strings −47, guitar −41, bass −39, keys −39, organ −37, drums +18. This is the vs-reference level with a human anchor; not the north star.
- *Tier H run on the proven corpus (6/6).* 40 works, **141 tasks**, 0 failures, 0 unavailable, 0 digest mismatches, judge 1.1. Reference composer mean **43.35 vs the human's own part 94.95** on the same windows (Δ −51.6; before, on the score-licence corpus: 49.21 vs 93.36, Δ −44.15 — different works, same judge and task, not a paired comparison); at or above the human on **0.7 %** of tasks (1 of 141 — Sweelinck's Psalm 90, reed; the old 3 of 153 were all drums, and the proven corpus has no drum parts); playability errors 1.26 per task (was 0.72), chord-tone share 0.94 (was 0.96). By target family (tasks, Δ): strings 33 −50.1, brass 26 −48.8, keys 21 −44.1, reed 20 −53.0, ensemble 12 −84.3, pipe 11 −44.3, organ 9 −39.5, guitar 7 −63.7, bass 2 −34.2. The gap widened on this material: polyphonic string and choral writing is harder for a chord-tone stacker than pop scores were.
- *Corpus coverage (6/6).* 40 of 100 songs; every required musical value ≥ 3 except `idiom "non_western"` **0 of 3** (no proven-public-domain multitrack work in PDMX carries a non-western label; the four that did — guantanamera, tong hua, Procuro Olvidarte, Dont Let Me Go — are contested) and what a score corpus cannot carry — `full_song` / `piano_vocal` / `vocal_only` 0 of 3, `electronic` production 0 of 3, human gold 0 of 30 — **7 gaps**, recorded, not filled. Produced pop / dance / Mizrahi, electronic and other modern idioms cannot come from public-domain compositions at all; those rows need licensed or operator-owned recordings (Tier P is the first).

**Tests.** `arrangementBenchmark.test.ts` 10 (incl. the tritone positive control of the harmony measure, refusal across pins/tiers/render/corpora, Tier P as its own run), `benchmarkMeasures.test.ts` 7, `positiveControlLedger.test.ts` 7, `realCorpusBenchmark.test.ts` 8 (measurement, refusal on metre changes, rarest-first selection, **contested and score-licence-only works excluded before the quota fill and reported with reasons; one entry per composition**, N-family stripping, the run with a silent provider scoring a real zero), `compositionRights.test.ts` 5 (list consistency — every composer dead by 1955; spelling folds; ABBA / Coolio / Croce / Fernández / Misc Computer Games / medleys / placeholders contested with reasons; traditional label not proof — Santa Baby refused, Greensleeves and a labelled chant admitted; arranger credits), `benchmarkCorpusPlan.test.ts` 6 (a public-domain claim on the score's licence alone refused, Sibelius d. 1957 refused, contested refused, **the generated Tier H holds no contested entry and every entry proves its composition**), `pdmxIngest.test.ts` 6 (a row with no verified composer refused as contested), `blindListening.test.ts` 6 (no controls → withheld; controls not heard → withheld `not_sensitive`; five raters → `insufficient_data`; eight raters detecting → allowed; control votes outside Elo/tallies; context bytes identical across controls; leak probes) — 55 tests, all registered in `run-focused-api-tests.mjs`; the existing `tournamentListening`, `listeningBenchmarkV2`, `listeningSensitivity` suites still pass (13). `pnpm run typecheck` green.

**Honest limits.** `shippedCriticScore` differs from `criticScore` by 0.11 on this sha because the perform stage does not yet repair notes (PR-98) and the critic is 76 % plan-derived: the number is in place, the inflation it is meant to show is not yet on main — re-snapshot after PR-98. `harmonyScore` near its ceiling on every corpus measures chord membership and semitone clashes, not voice leading; it is discriminating (tritone control) but flatters label-stacking. `seamArtefacts` and `trajectorySmoothness` — two of the three coherence components that replaced the constants — detect no corruption family; they are measurements, not validated gates. `candidateDistance` ≈ 6.6/100 on every case because the gate's plan-level components are empty on orchestrator output. `renderFailures = 5` on every render case (every reference-synth render reported infeasible while the audio critic scores it 87–93) is recorded, not investigated — outside B-08's files. The ledger's detection is a strict inequality on deterministic metrics: a gate verdict says the metric sees the damage, not that it sizes it; `partJudge.score` on arrangement anchors compares a part to its own original and is biased towards detection. `musicCritic` and `candidateQuality` could only be run on Tier S anchors (they need a plan). **Tier H rights (corrected 6/6):** every entry now proves its composition beside the uploader's score statement, but the proof rests on PDMX's `composer_name` / `artist_name` as typed — a public-domain work whose row names nobody, or spells its composer in a way the curated list lacks (Domenico Micheli, Bernardino Mosto, Joseph Key, the village-band manuscripts under "Misc tunes"), is contested and **under-admitted, never guessed in**; the list is extendable with a death year and a re-run. A single-word handle beside a verified title is treated as naming nobody ("nordalmeida1" beside Silent Night); a multi-word modern name blocks. The verified tune list admits a title only when the row names nobody or says traditional, and liturgical texts need the label too — a modern original sharing a distinctive public-domain title would still pass on the title. The term is life + 70 (dead by 1955): under the United States' publication-based term, works first published after 1929 by composers who died 1930–1955 may still be protected there — the corpus carries two such entries (Vierne d. 1937: Benedictus, Symphonie No. 1) and the lead should decide whether a US-facing product excludes them. The tournament evidence of PR-76/77 (`model-tournament-global-live.json`, 47 of 50 works contested here) and the Listening V2 report were **not** re-run or edited in this stream; only Tier H and the ledger's Tier H anchors changed. Non-western idiom is now a recorded gap (0 of 3); `feel: swung` on this corpus is notated triplet subdivision in slow classical movements. The Tier H run numbers before and after are on different works and are not a paired comparison. Swing is read from notated onset phases, idiom from PDMX labels, harmony complexity from the CSV's pitch-class entropy; `feel: swung` therefore under-counts. The Tier H task is N = 1 in the committed run. With three control pairs a Gate C session needs eight independent raters answering every pair before both gate rungs have their eight votes; the control pairs are rendered with `LISTENING_SYNTH_V2` while the song pair plays the candidates' evaluation renders (one renderer within a pair, not across pairs). The route wiring was typechecked, not run — no database or server in this stream; no human has voted on any control pair; nothing here trains anything or changes the training gate.
...
- **Real corpus (audit §7.2).** `REAL_BENCHMARK_CORPUS` holds 40 Tier H entries generated by `scripts/select-real-corpus.mjs` into `realCorpusTierH.ts` (evidence `docs/evidence/real-corpus-tier-h-selection.json`): the whole admitted PDMX table streamed (254,077 rows, 222,856 admitted, 25,414 multitrack), every multitrack MIDI on disk **measured** (`realCorpusSelection.ts`: one metre per work or refused, first tempo, bars, parts, families, onsets per bar, swing from off-beat onset phases; genre/tags for idiom; CSV pitch-class entropy for harmony), 7,633 candidates, **then the composition-rights step (6/6, below — only 901 candidates prove their composition)**, then the repo's tournament-anchor works first and every required value filled rarest-first to `MIN_PER_VALUE` from the proven works, one entry per composition: 4/4 ×19, 3/4 ×8, 3/8 ×3, 6/8 ×3, 12/8 ×2, 3/2 ×2, 2/2 ×2, 2/4 ×1; swung ×3 (notated triplet subdivision, not a jazz feel), compound ×5; dense ×25, moderate ×12, sparse ×3; large ×25, small ×15; slow ×21, medium ×15, fast ×4; complex ×11, moderate ×21, simple ×8; **non_western 0 of 3** (a gap, not filled). `CorpusEntry.symbolicSource` (work id, path, sha256, measurements, how the repo admitted it). The **Tier H task** is stated once (`TIER_H_TASK`, "strip N families → arrange → compare against the human original at the part level", tournament task shape) and implemented: `runRealCorpusBenchmark` (`pnpm … run real-corpus` → `docs/evidence/real-corpus-tier-h-run.json`) runs the reference composer on every entry with material, 16-bar windows, one task per GM program, judged by `judgePart` against the human anchor; `stripContextFamilies` implements N > 1. `PROFESSIONAL_HUMAN_GOLD` stays blocked (`levelBlocker` unchanged). **Tier P**: the owner's song "רחם נא" as a slimmed fixture (`__fixtures__/rachemNaSongModelV3.ts`, 91 KB: tempo/meter/key maps, 92 chords, 9 sections, 141 bars, beats, energy, vocal evidence, the musical map's harmony/energy/vocals summaries; project ids, reconciliation, provenance and storage paths dropped), rights `owned_by_operator`, `--tier P` runs it as its own run; tiers never compare.
- **Human A/B controls (audit §7.5).** `blindListening.ts` 1.1: every candidate session carries three `HUMAN_DEGRADED` control pairs (pitch_shift 60 %, 30 %, 60 %) whose pair ids, case ids and questions are indistinguishable from the song pair's (the comparison lives in owner-only `meta`; the leak-probe list passes); `buildCandidateControls` writes the challenger's tracks as one MIDI with the most active pitched track last and degrades only that track through `degradeSideMidi` (context bytes identical across every control); `sessionResults` **withholds Gate C** unless `sensitivityReport(session, independent votes, primary question).gate.verdict === "may_judge_training"` — a session with no control pairs is withheld with that reason — and control votes never enter the Elo, the tallies or the release share. `gateC.sensitivity` added to the result and to `ListeningGateC` in `openapi.yaml` (orval regenerated). `POST /projects/{id}/listening-sessions` builds the pairs, renders original + degraded copies with `LISTENING_SYNTH_V2`, stores them under the session (`audioByToken`); when the challenger has no track models or the project no Song Model tempo the session is created without controls and Gate C says so.
- **Rights correction (6/6, the lead's ruling of 2026-09-10).** The 40 Tier H entries of 3/5 were admitted on the PDMX row's CC0 / public-domain statement alone. That statement is the **uploader's**, and clears at most the engraving or arrangement — never the composition ("Mamma Mia!" under CC0 is still ABBA's song); the standing rule (charter §2.2, `benchmarkCorpusPlan.ts`: a dataset's licence is not proof of rights in the works inside it) was being broken. The corpus now **fails closed on the composition**: `CorpusRightsBasis` is a union — `public_domain` requires `composer` + `composerDied` (year ≤ `PUBLIC_DOMAIN_DEATH_YEAR_CUTOFF` = 2026 − 70 − 1 = **1955**) or `traditional: true` + `source`; a new `contested` kind (`commercialUse: false`, `reason`) records a work the corpus looked at and could not clear. `corpusRefusalReason` refuses a public-domain claim with no composition evidence ("claims public domain on the score's licence alone"), a composer dead after the cutoff, a traditional claim with no source, and every contested basis. The mechanism is **admit only what is proven**, never a denylist: new `compositionRights.ts` holds a curated list of **301 public-domain composers** with death years and the spellings met in PDMX (Renaissance polyphonists through Ravel / Holst / Vierne / Rachmaninoff / Prokofiev; nothing after 1955 — Sibelius d. 1957 is absent) and **98 verified traditional / public-domain tunes** (Silent Night → Gruber d. 1863, Adeste Fideles → Wade, Greensleeves → English 1580, Jingle Bells → Pierpont …); `compositionRightsFor` reads the row's `composer_name` / `artist_name` (new `PdmxCsvRow.artist`) and title — dates, keys, punctuation and diacritics folded, `arr.` credits split off — and returns a basis or a reason (a multi-word name the list does not know blocks; a single-word handle names nobody but does not block a verified title; a "Traditional" / "Misc Christmas" label is not proof unless the title is on the tune list — "Santa Baby" and "Mary Did You Know" arrive under those labels; "Misc Computer Games" is a category, not a composer). `pdmxRightsBasis` now returns `public_domain` with the composition basis or `contested`; `pdmxToCorpusEntries` refuses contested rows with the reason; `selectTierH` 2.0 runs `admitEntries` **before any quota is filled** and returns `excluded[]` — a contested work leaves its slot open or short, never filled by the next unproven work — and collapses further arrangements of one composition (the first proven-only run held two "Stille Nacht", two "Canon and Gigue"). Both layers are stated per entry: the composition through the basis, the arrangement / engraving through the uploader's dedication in `no_license_conflict` — a living arranger's CC0 arrangement of a public-domain work is admitted on those two facts; an arrangement of an unproven composition is contested whatever its score licence. **Result:** of 7,633 measured candidates **901 prove their composition** (872 by composer, 29 by tune) and **6,732 are contested** (4,661 name a person not on the list, 991 carry only a handle, 742 name nobody, 199 wear a traditional label on an unlisted title, 139 a category label); **36 of the previous 40 entries and 49 of the 54 tournament seeds are contested** and listed in the report's `excluded[]` with title and reason — "Mamma Mia!" (names Andersson / Anderson / Ulvaeus), "Gangsta's Paradise" (Rickey C Battle Jr, the arranger, in the composer field), "Señorita" (Camila Cabello), "i will survive" (Gloria Gaynor), "country roads" (Hermes House Band), "time in a bottle" (Jim Croce), "earth angel" (Marvin Berry / The Starlighters), "sandstorm" (Darude), "Nipsey Hussle x Victory Lap" (Tyquon Garrett), "guantanamera" (Joseíto Fernández, d. 1979), "let me down slowly" (Alec Benjamin / Alessia Cara), "Pure Cocaine" (Lil Baby), "I Can't Breathe" (Jordan Jennings), "earthbound - onett" (Misc Computer Games); the four kept are Silent Night (Gruber), Symphony No. 9 (Beethoven), Vierne's Benedictus and Sullivan's Christ is Risen. Every contested candidate of the scan is in the sidecar `docs/evidence/real-corpus-tier-h-contested.json` (6,732 rows, composer / artist / reason), so the gap is visible rather than filled. **The new Tier H: 40 works, 31 composers** — Renaissance 7 (Dufay d. 1474, Lasso, Pallavicino, G. Gabrieli, Sweelinck, Leoni, d'India), Baroque 11 (Huygens, Purcell, Pachelbel, Buxtehude, Vivaldi, Bach ×5, Handel), Classical 7 (Mozart ×4 — K.467, K.477, K.618, K.626 — Haydn, Beethoven, Schubert), Romantic and after 13 (Gruber, Mussorgsky, Wagner, Liszt, Gounod, Verdi, Sullivan, Parry, Saint-Saëns ×2, Fauré, Vierne ×2), traditional 2 (Greensleeves, Bring a Torch); latest death year 1937; 35 from the CSV scan, 5 tournament seeds. The positive-control ledger's Tier H anchors now come from this corpus (`run-control-ledger.mjs --source corpus`, default: the Tier H task windows of every entry, 2 per work) instead of the Listening V2 report's 12 score-licence works. The idiom regex lost its bare `world` (it had labelled three Christmas carols non_western through the tag "joytotheworld") and is bounded by non-letters; under-counting is the honest direction.
