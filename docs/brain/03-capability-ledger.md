...
| Arrangement arc (intended dynamics, texture, tension roles, entries/exits) | D | **V**: the arc is realised in notes. B-21 writes the opening figure and the ending gesture; the owner's first note moved 3.795 s → **0.000 s** and the texture follows the arc's per-section level | PR-B01, PR-B21 | the ritardando the arc asks for is still not applied (performance stage) |
...
| Register plan applied to pitch choice | D | N + T for the *instrument's* ceiling (B-21 reads `roleRegisterFor`; the owner's register dimension 66.3 → 100). **Still N-pending for the voice**: B-26 measured that obeying the ceiling pushes beds onto the singer (pop-full masking 2 → 4) and added `vocalRegisterConflict`, which **no writer reads** — 9 `no_room` across the anchors | PR-B03, PR-B21, PR-B26 | a writer must arbitrate ceiling vs voice |
...
| Repair that changes notes and names the origin layer; backtracking | — (plan-only, discarded) | **N + T for three operators**: `revoice_to_chord_tones`, `requantise_part`, `lower_top_voice_to_ceiling`, each with a positive control and a refusal when the verdict would get worse. On the owner's song `clashShare` 0.4857 → 0.1302, `chordToneShare` 0.416 → 0.8698, blocking → minor, judge burden 602.72 → 423.73. **30 of 33 operations the critics name are still unimplemented** | PR-B20 | the remaining 30, listed by name in its evidence |
...
| Critics' anchors survive the arrangement improving | — (anchors recomposed from live code; every pin a post-repair number) | N + T: each control re-pointed at a constructed defect when the owner's song stopped carrying its defect; the anchor builders no longer run the repair stage (B-20's finding) | PR-B25, PR-B26 | unify the two anchor builders |
